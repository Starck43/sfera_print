...
export const useZoomRegion = () => {
	const [
		zoomedRegion,
		setZoomedRegion
	] = useState<{ next?: ZoomedRegion, prev?: ZoomedRegion } | null>(null)
	...
	const updateZoomedRegion = useCallback((region: ZoomedRegion | null) => {
		...
		setZoomedRegion((state) => {
			if (!region && state?.next) return {
				next: undefined,
				prev: {...state.next, onComplete: () => updateZoomedRegion(null)}
			}
	...
	const renderRegion = useCallback((handleClick: (e, regionId?: number) => void) => (zoomedRegion: ZoomedRegion | undefined, style: string) => {
		...
		return (
			...
			>
				...
				{zoomedRegion.cities?.map((city, idx) => renderCity()
				(city, zoomedRegion, 1 - (zoomedRegion.cities.length - 1 - idx) * .2))
				}
			</g>
		)
	}, [renderCity])
