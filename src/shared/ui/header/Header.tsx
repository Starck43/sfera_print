...
export const Header = memo((props: HeaderProps) => {
    ...
    const subtitle = typeof subTitle === "string" ? <span className={cls.subtitle}>{subTitle}</span> : subTitle

    const Title = tag
    const content = (
        <Title
            className={classnames(cls, ["title", align, transform], { shadowed, fullWidth })}
            style={style}
        >
            {title}
        </Title>
    ...
    return href ? (
        <Link href={href} className={classnames(cls, ["header", "link", status, size], {}, [className])}>
            {content}
        </Link>
    ) : (
        content
